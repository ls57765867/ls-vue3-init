...
export const handleNetworkError = (errStatus?: number): void => {
  ...
  if (errStatus) {
    ElMessage.error({ message: networkErrMap[errStatus] ?? `其他连接错误 --${errStatus}` })
    return
  }

  ElMessage.error({ message: '无法连接到服务器！' })
}

export const handleAuthError = (code: number): boolean => {
  const authErrMap: Error = {
  ...
  if (Object.prototype.hasOwnProperty.call(authErrMap, code)) {
    ElMessage.error({ message: authErrMap[code] })
    // logout();
    return false
  }
...
export const handleGeneralError = (code: number, errmsg: string): boolean => {
  if (code !== 200) {
